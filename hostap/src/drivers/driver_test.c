...
/*
 * Function pointer to message sending callback.
 * If this pointer is initialized, the callback intercepts all EAPOL messages which are going out of Unix socket
 */
int (*test_driver_send_eapol_cb)(void *drv, const u8 *data, size_t data_len) = NULL;

void test_driver_set_send_eapol_cb( int (*send_eapol_cb)(void *drv, const u8 *data, size_t) )
{
	test_driver_send_eapol_cb = send_eapol_cb;
}
